...
public final class MultiUpdateBase extends Defines {
	private static Log log = Log.getLog(MultiUpdateBase.class.getName());
	...
	public MultiUpdateBase(CacheClientManager manager, int groupID, TransCoder transCoder) {
	...
	public int multiUpdateBase(List<MultiUpdateBaseItem> list, byte subOp) {
		lastError = null;
		opFlag = (byte)(subOp | XIXI_UPDATE_BASE_REPLY);
		if (list == null) {
			lastError = "multiUpdateBase, list == null";
			log.error(lastError);
			return 0;
		}
		if (list.size() == 0) {
			lastError = "multiUpdateBase, list.size() == 0";
			log.error(lastError);
			return 0;
		}
		Map<String, Connection> conns = new HashMap<String, Connection>();
		try {
			Iterator<MultiUpdateBaseItem> it = list.iterator();
			int index = 0;
			while (it.hasNext()) {
				MultiUpdateBaseItem item = it.next();
				if (item == null) {
					lastError = "multiUpdateBase, item == null";
					log.error(lastError);
					return 0;
				}

				if (item.key == null) {
					lastError = "multiUpdateBase, item.key == null";
					log.error(lastError);
					return 0;
				}
				...
				if (host == null) {
					lastError = "multiUpdateBase, can not get host with the key";
					log.error(lastError);
					return 0;
				}
			...
			while (numConns > 0 && timeRemaining > 0) {
				...
				if (n > 0) {
					...
					lastError = "multiUpdateBase, selector timed out";
					log.error(lastError);
					break;
				}
			...
			lastError = "multiUpdateBase, exception on " + e;
			log.error(lastError);
			return 0;
		} finally {
			try {
	...
	private final class Connection {
		...
		private ArrayList<MultiUpdateBaseItem> items = new ArrayList<MultiUpdateBaseItem>();
		private ArrayList<Integer> itemIndexs = new ArrayList<Integer>();
		private int currKeyIndex = 0;

		public void add(MultiUpdateBaseItem item, Integer index) {
			items.add(item);
			itemIndexs.add(index);
		}

		MultiUpdateBaseItem item = null;
		byte[] keyBuf = null;
