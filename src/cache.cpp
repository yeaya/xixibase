...
xixi_reason Cache_Mgr::add(Cache_Item* it, uint64_t&/*out*/ cache_id) {
  xixi_reason reason;
  ...
  if (old_it == NULL) {
    do_link(it);
    stats_.set_success(it->group_id, it->class_id);
    reason = XIXI_REASON_SUCCESS;
    cache_id = it->cache_id;
  } else {
    do_release_reference(old_it);
    stats_.add_fail(it->group_id, it->class_id);
...
xixi_reason Cache_Mgr::set(Cache_Item* it, uint64_t&/*out*/ cache_id) {
  xixi_reason reason;
  ...
  Cache_Item* old_it = do_get(it->group_id, it->get_key(), it->key_length, it->hash_value_);
  if (old_it != NULL) {
//    LOG_ERROR("Cache_Mgr set, key " << string((char*)it->get_key(), it->key_length) << " hash_value=" << it->hash_value);
    if (it->cache_id == 0 || it->cache_id == old_it->cache_id) {
      ...
      stats_.set_mismatch(it->group_id, it->class_id);
...
xixi_reason Cache_Mgr::replace(Cache_Item* it, uint64_t&/*out*/ cache_id) {
  xixi_reason reason;
...
xixi_reason Cache_Mgr::append(Cache_Item* it, uint64_t&/*out*/ cache_id) {
  xixi_reason reason;
  ...
  if (old_it != NULL) {
    if (it->cache_id != 0 && it->cache_id != old_it->cache_id) {
      ...
      if (new_it != NULL) {
        ...
        stats_.append_success(it->group_id, it->class_id);
        reason = XIXI_REASON_SUCCESS;
...
xixi_reason Cache_Mgr::prepend(Cache_Item* it, uint64_t&/*out*/ cache_id) {
  xixi_reason reason;
  ...
  if (old_it != NULL) {
    if (it->cache_id != 0 && it->cache_id != old_it->cache_id) {
      ...
      if (new_it != NULL) {
        ...
        new_it->cache_id = old_it->cache_id;
        do_replace(old_it, new_it);
        stats_.prepend_success(it->group_id, it->class_id);
        reason = XIXI_REASON_SUCCESS;
        cache_id = new_it->cache_id;
        do_release_reference(new_it);
      } else {
