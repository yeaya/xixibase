...
void set_log_level(int log_level);
  ...
  #if LOG_LEVEL <= log_level_trace
#define LOG_TRACE(x)  if (log_level_ <= log_level_trace) { log_lock_.lock(); std::cout << x << "\n"; log_lock_.unlock(); }
  #else
    #define LOG_TRACE(x)
  #endif

  #if (LOG_LEVEL <= log_level_debug)
    #define LOG_DEBUG(x)  if (log_level_ <= log_level_debug) { log_lock_.lock(); std::cout << x << "\n"; log_lock_.unlock(); }
  #else
    #define LOG_DEBUG(x)
  #endif

  #if LOG_LEVEL <= log_level_info
    #define LOG_INFO(x)    if (log_level_ <= log_level_info) { log_lock_.lock(); std::cout << x << "\n"; log_lock_.unlock(); }
  #else
    #define LOG_INFO(x)
  #endif

  #if LOG_LEVEL <= log_level_warning
    #define LOG_WARNING(x)  if (log_level_ <= log_level_warning) { log_lock_.lock(); std::cout << x << "\n"; log_lock_.unlock(); }
  #else
    #define LOG_WARNING(x)
  #endif

  #if LOG_LEVEL <= log_level_error
    #define LOG_ERROR(x)  if (log_level_ <= log_level_error) { log_lock_.lock(); std::cout << x << "\n"; log_lock_.unlock(); }
  #else
    #define LOG_ERROR(x)
  #endif

  #if LOG_LEVEL <= log_level_fatal
    #define LOG_FATAL(x)  if (log_level_ <= log_level_fatal) { log_lock_.lock(); std::cout << x << "\n"; log_lock_.unlock(); }
  #else
    #define LOG_FATAL(x)
  #endif
#endif
