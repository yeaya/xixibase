...
Peer_Cache::Peer_Cache(boost::asio::ip::tcp::socket* socket) :
    read_pdu_(NULL),
...
void Peer_Cache::process_update_req_pdu_extras(XIXI_Update_Req_Pdu* pdu) {
  ...
  switch (pdu->sub_op()) {
    case XIXI_UPDATE_SUB_OP_SET:
      reason = cache_mgr_.set(cache_item_, cache_id);
      break;
    case XIXI_UPDATE_SUB_OP_ADD:
      reason = cache_mgr_.add(cache_item_, cache_id);
      break;
    case XIXI_UPDATE_SUB_OP_REPLACE:
      reason = cache_mgr_.replace(cache_item_, cache_id);
      break;
    case XIXI_UPDATE_SUB_OP_APPEND:
      reason = cache_mgr_.append(cache_item_, cache_id);
      break;
    case XIXI_UPDATE_SUB_OP_PREPEND:
      reason = cache_mgr_.prepend(cache_item_, cache_id);
      break;
    default:
