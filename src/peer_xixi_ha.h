...
#ifndef PEER_XIXI_HA_H
#define PEER_XIXI_HA_H
...
// Peer_XIXI_HA
//
class Peer_XIXI_HA : public Peer {
public:
  Peer_XIXI_HA(boost::asio::ip::tcp::socket* socket);
  virtual ~Peer_XIXI_HA();
...
protected:
  ...
  // present req
  inline uint32_t process_ha_present_req_pdu_extras(XIXI_HA_Present_Req_Pdu* pdu, uint8_t* data, uint32_t data_length);

  // create committee req
  inline uint32_t process_ha_create_committee_req_pdu_extras(XIXI_HA_Create_Committee_Req_Pdu* pdu, uint8_t* data, uint32_t data_length);

  // announce leader req
  inline uint32_t process_ha_announce_leader_req_pdu_extras(XIXI_HA_Announce_Leader_Req_Pdu* pdu, uint8_t* data, uint32_t data_length);

  // keepalive req
  inline uint32_t process_ha_keepalive_req_pdu_extras(XIXI_HA_Keepalive_Req_Pdu* pdu, uint8_t* data, uint32_t data_length);

  // MUTEX create
  inline uint32_t process_ha_mutex_create_req_pdu_extras(XIXI_HA_Mutex_Create_Req_Pdu* pdu, uint8_t* data, uint32_t data_length);

  // MUTEX lock
  inline uint32_t process_ha_mutex_lock_req_pdu_extras(XIXI_HA_Mutex_Lock_Req_Pdu* pdu, uint8_t* data, uint32_t data_length);

  // MUTEX unlock
  inline uint32_t process_ha_mutex_unlock_req_pdu_extras(XIXI_HA_Mutex_Unlock_Req_Pdu* pdu, uint8_t* data, uint32_t data_length);

  // MUTEX destory
  inline uint32_t process_ha_mutex_destroy_req_pdu_extras(XIXI_HA_Mutex_Destroy_Req_Pdu* pdu, uint8_t* data, uint32_t data_length);
  ...
  static void destroy(Peer_XIXI_HA* peer);
